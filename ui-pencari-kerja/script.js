...
document.getElementById("filter-kata-kunci").addEventListener("input", 
renderPencarian); 
...
function renderPencarian() { 
  ...
  let data = lowonganData.filter(l => 
    (!kata || `${l.posisi} ${l.lokasi} ${l.perusahaan}`.toLowerCase().includes(kata)) && (!bidang || l.bidang === bidang) 
  ); 
