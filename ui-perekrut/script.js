// =====================================================================
// LinkKerja — UI Perekrut (contoh data di memori, belum tersambung API)
// Dikerjakan oleh: Nabil Sunu Widyadhana (254311058)
// =====================================================================

// --- contoh data, cerminan tabel `lowongan` & `lamaran` di database ---
let lowonganList = [
...
function labelTipe(t) {
  return { full_time: "Full-time", part_time: "Part-time", magang: "Magang", kontrak: "Kontrak" }[t] || t;
}
...
function renderDashboard() {
  document.getElementById("stat-lowongan").textContent = lowonganList.filter(l => l.status === "aktif").length;
  document.getElementById("stat-lamaran").textContent = kandidatList.length;
  document.getElementById("stat-ditinjau").textContent = kandidatList.filter(k => k.status === "ditinjau" || k.status === "terkirim").length;
  document.getElementById("stat-diterima").textContent = kandidatList.filter(k => k.status === "diterima").length;
